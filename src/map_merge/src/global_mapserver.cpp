...
static int maximum_iterations = 150;
static double transformation_epsilon = 0.01;
static double max_correspondence_distance = 0.5;
...
static geometry_msgs::TransformStamped tr;

static ros::Publisher filter_map_pub;
...
static std::string _localizer = "odom";
static std::string _offset = "linear";  // linear, zero, quadratic

static std::ofstream ofs;
static std::string filename;

static void param_callback(const map_merge::ConfigICP::ConstPtr& input)
{
...
static void initialpose_callback(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr& input)
{
  ...
  try
  {
    ...
    listener.waitForTransform("/odom", "/odom", now, ros::Duration(10.0));
    listener.lookupTransform("/odom", "/odom", now, transform);
  }
...
static void points_callback(const sensor_msgs::PointCloud2::ConstPtr& input)
{
  if (map_loaded == 1 && init_pos_set == 1)
  {
    ...
    pcl::PointCloud<pcl::PointXYZ>::Ptr filtered_scan_ptr(new pcl::PointCloud<pcl::PointXYZ>(filtered_scan2));
		...
		pcl::toPCLPointCloud2(*filtered_scan_ptr, point_cloud2);

		pcl_conversions::fromPCL(point_cloud2,output2);
		output2.header.frame_id="camera_depth_optical_frame";
		filter_map_pub.publish(output2);
    ...
    estimated_vel_pub.publish(estimate_vel_msg);
		...
		tr.header.frame_id = "/camera_depth_optical_frame";
    tr.header.stamp = current_scan_time;
    tr.transform.translation.x = icp_pose.x;
    tr.transform.translation.y = icp_pose.y;
    tr.transform.translation.z = icp_pose.z;
    tr.transform.rotation.x = icp_q.x();
    tr.transform.rotation.y = icp_q.y();
    tr.transform.rotation.z = icp_q.z();
    tr.transform.rotation.w = icp_q.w();

  	tf2::doTransform (*input, output, tr);
  	global_map_pub.publish (output);
...
int main(int argc, char** argv)
{
  ...
  icp_stat_pub = nh.advertise<map_merge::ICPStat>("/icp_stat", 1000);
	global_map_pub = nh.advertise<sensor_msgs::PointCloud2>("/global_map", 1);
	filter_map_pub = nh.advertise<sensor_msgs::PointCloud2>("/filter_map", 1);

  // Subscribers
  ros::Subscriber param_sub = nh.subscribe("config/icp", 10, param_callback);
  //ros::Subscriber gnss_sub = nh.subscribe("gnss_pose", 10, gnss_callback);
  ros::Subscriber map_sub = nh.subscribe("points_raw1", 10, map_callback);
